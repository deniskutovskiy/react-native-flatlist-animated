import React, { useEffect, useRef, ForwardedRef, useLayoutEffect } from "react";
...
const AnimatedFlatList = React.forwardRef(function (
  {
    data,
    renderItem,
    itemContainerHeight,
  ...
  const oldKeys = oldData.current.map((item) => item.key);
  ...
  const renderAnimatedItem = ({ item, index, separators }) => {
    let hasExitingValue: boolean = oldKeys[index] && !keys.includes(oldKeys[index]);
    let hasEnteringValue: boolean = !oldKeys.includes(item.key);

    let translateFrom: number,
      translateTo: number = index;
