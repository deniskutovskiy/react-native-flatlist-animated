...
interface IAnimatedItemProps {
  ...
  export function AnimatedItem({
    ...
    let enterStyles, exitStyles, swapStyles, translateStyles;
    if (hasEnteringValue) {
      enterStyles = EnteringAnimations[selection.entering](
        animationProps,
        selection
      );
    } else {
      if (selection.reordering !== "none" && shouldSwapValue) {
        swapStyles = ReorderingAnimations[selection.reordering](
          animationProps,
          selection
        );
      } else {
        translateStyles = LayoutTransitions[
          animationProps.horizontal ? "horizontal" : "vertical"
        ](animationProps, selection);
      }
    }
  
    if (hasExitingValue) {
      exitStyles = ExitingAnimations[selection.exiting](
        animationProps,
        selection
      );
    }
